...
export async function listRuns(base) {
  ...
  // Newest first within a state. The id was the only tiebreak, which made the
  // order alphabetical among the runs of one PR: reviewing #3 five times put
  // the first attempt on top and left the console opening a stale run by
  // default. `attempt` is the run's own stamp; runs made before it existed sort
  // last among their peers rather than jumping the queue.
