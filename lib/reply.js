...
import { withModel } from "./agents.js";
 ...
 */
export function buildReply({ agent, thread, sha, worktree, quotePrior, stopToken, others = [], context = "" }) {
...
including where I disagree. The current state is at ${worktree}; reviewed heads: ${sha}.
${context}
...
export function replyArgv(agent, { promptText, promptFile, worktree, sha, sessionId }) {
  ...
  const vars = { promptText, promptFile, worktree, sha, sessionId: sessionId ?? undefined };
  return {
    ...
    argv: withModel(argv, agent).map((a) =>
