 ...
 * something.
 */
export function triagePrompt({ finding, trunk, stopToken, context = "" }) {
  return `${context}

A code reviewer raised this finding against the current worktree.
...
1. Read the code at that location. ${context ? "Use the per-PR diff commands above. Paths beginning PR1/, PR2/, etc. refer to the mapped checkouts. Fix each affected PR in its own checkout." : `Run git diff from the merge base with origin/${trunk} to HEAD if you need to see what changed.`}
