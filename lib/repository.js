...
/** Whether a repository already contains a specific commit. */
async function hasCommit(dir, sha, { exec }) {
  if (!dir || !sha) return false;
  try {
 ...
 */
async function localHead(dir, target, number, { exec, namespace }) {
  if (!dir) return null;
  try {
...
export async function resolvePrCheckout(prUrl, {
  ...
  // The head oid comes from `gh`, so unlike a merge request there is a known
  // commit to look for: prefer the caller's own repository when it has it.
  const local = await localHead(dir, target, number, { exec, namespace: "refs/pull" })
    ?? (dir && await hasCommit(dir, details.headRefOid, { exec }) ? details.headRefOid : null);
  try {
    if (local) {
      await exec("git", ["clone", "--quiet", "--no-checkout", dir, worktree]);
