 ...
 * others.
 */
export async function runAgent(agent, { worktree, prompt, stopToken, dryRun, onLog, timeoutSeconds }) {
  ...
  if (agent.promptDelivery === "file") {
    tmp = await mkdtemp(path.join(tmpdir(), "macr-"));
    promptFile = path.join(tmp, "prompt.md");
    await writeFile(promptFile, prompt, "utf8");
  }

  const vars = { worktree, promptFile: promptFile ?? "", promptText: prompt };
  const [cmd, ...args] = subst(agent.argv, vars);
  const cwd = agent.cwd === "worktree" ? worktree : process.cwd();

  onLog?.(`${agent.name}: ${cmd} (${agent.cwd === "worktree" ? "in worktree" : "via flag"})`);
...
function speaking(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (said.length && said.every((s) => /^(?:FINDING|WHERE)\s*:/i.test(s))) {
...
export async function probe(agent) {
  ...
  // would fail `macr agents` for an agent that is by definition present.
