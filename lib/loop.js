  ...
  for (const e of events) {
    switch (e.t) {
      ...
      case "agent.alive": {
        ...
        // one.
        const w = t.turns.find((x) => x.kind === "waiting" && x.round === e.round);
        if (w) { w.seconds = e.seconds; break; }
        // Real output already supersedes it; do not regress to a placeholder.
        if (t.turns.some((x) => (x.kind === "streaming" || x.kind === "report") && x.round === e.round)) break;
