    ...
    switch (e.t) {
      case "finding.raised":
        ...
        thread(e.agent).turns.push({
          who: e.agent, kind: "finding", id: e.id, round: e.round,
          claim: e.claim, loc: e.loc ?? "", body: e.body ?? "", ts: e.ts,
        });
        break;
      ...
      case "finding.resolved":
        ...
        break;
      case "finding.turn":
        thread(claims.get(e.id)?.agent ?? e.agent ?? "?").turns.push({
          who: e.who ?? "?", kind: "rebuttal", id: e.id, text: e.text ?? "", ts: e.ts,
        });
        break;
      case "agent.chunk":
        ...
        // per token.
        {
          ...
          else t.turns.push({ who: e.agent, kind: "streaming", round: e.round, text: e.text, ts: e.ts });
        }
        break;
      case "agent.report":
        {
          ...
          const i = t.turns.findIndex((x) => x.kind === "streaming" && x.round === e.round);
          const turn = {
 ...
 */
export async function replyRound({ dir, pool, cfg, worktree, sha, round, findings, dryRun, onLog, onChunk }) {
  ...
  const out = await Promise.all(pool.map(async (a) => {
    ...
    const { argv, resumed } = replyArgv(a, { promptText: text, worktree, sha });
    await appendEvent(dir, { t: "reply.sent", agent: a.name, resumed, text });

    const r = await runAgent({ ...a, argv }, {
      worktree, prompt: text, stopToken: cfg.stopToken, dryRun, onLog,
