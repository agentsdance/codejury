...
export const DEFAULTS = {
  ...
  agents: [
    ...
    {
      name: "kimi",
      product: "Moonshot Kimi Code",
      role: "reviewer",
      promptDelivery: "argv",
      cwd: "worktree",
      argv: ["kimi", "--work-dir", "{{worktree}}", "--quiet", "--plan", "--prompt", "{{promptText}}"],
      judgeArgv: ["kimi", "--work-dir", "{{worktree}}", "--quiet", "--yolo", "--prompt", "{{promptText}}"],
      resume: { supported: false, reason: "fresh conversation with finding context; never implicitly continue a session" },
      report: "whole",
      expectSeconds: 600,
    },
    {
