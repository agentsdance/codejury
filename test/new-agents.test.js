...
const cases = [{ name: 'kimi', bin: 'kimi', review: ['--work-dir', '@cwd', '--quiet', '--plan', '--prompt', '@prompt'], judge: ['--work-dir', '@cwd', '--quiet', '--yolo', '--prompt', '@prompt'] }];
...
for (const spec of cases) test(`${spec.name}: installed discovery, literal invocation, roles, selection, and failures`, async t => {
  ...
  const agent = cfg.agents.find(a => a.name === spec.name);
  ...
  await script(clean+'process.exit(2);'); assert.match((await invoke(agent)).report,/exited 2/);
