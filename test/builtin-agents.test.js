...
// OpenCode shipped enabled in 0.5.x; these are the ones added opt-in here.
const NEW = ["qwen", "copilot", "cursor", "amp", "kimi"];
...
test("the new agents ship opt-in, so a default install needs no extra CLI", async () => {
  ...
  for (const name of ["codex", "claude", "grok", "droid", "opencode"]) {
