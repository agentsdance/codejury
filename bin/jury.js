...
import { loadConfig, reviewers, judgeAgent, knownAgents } from "../lib/config.js";
...
function selectReviewers(cfg, requested, judge = null) {
  ...
  if (problems.length) {
    const offer = [...eligible.map((a) => a.name), ...optIn.map((a) => a.name)];
    const available = offer.join(", ") || "none";
    throw new Error(
  ...
  return byName.size
    ? names.map((name) => byName.get(name)).filter(Boolean)
    : eligible.filter((a) => names.includes(a.name));
}
...
async function cmdAgents() {
  ...
  for (const p of found) {
    ...
    const optIn = byName.get(p.name)?.enabled === false ? "  (opt-in)" : "";
    console.log(
      `${p.ok ? "ok     " : "MISSING"} ${p.name.padEnd(9)} ${role.padEnd(8)} ${p.path ?? p.bin}${optIn}`,
    );
  }
  ...
  const optIn = found.filter((p) => byName.get(p.name)?.enabled === false);
  if (optIn.length) {
  ...
  // red on every machine that has not installed all ten CLIs.
