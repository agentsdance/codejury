...
import { loadConfig, reviewers } from "../lib/config.js";
...
const USAGE = `macr — multi-agent code review
  ...
  macr review [flags]     run one round: every reviewer reads the diff, findings are recorded
...
async function cmdAgents() {
  ...
  for (const p of found) {
    console.log(`${p.ok ? "ok     " : "MISSING"} ${p.name.padEnd(8)} ${p.path ?? p.bin}`);
  }
